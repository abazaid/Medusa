$token = (Invoke-RestMethod -Method Post -Uri "http://localhost:9000/auth/user/emailpass" -ContentType "application/json" -Body '{"email":"newadmin2@medusa-test.com","password":"admin456"}' | Select-Object -ExpandProperty token)

$headers = @{
    Authorization = "Bearer $token"
}

Write-Output "Getting new admin user info..."
